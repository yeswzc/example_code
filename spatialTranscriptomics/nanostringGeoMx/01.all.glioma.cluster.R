setwd("/Users/wuz6/Documents/Project/08.spatialTranscriptome/01.analyze/01.cluster/")
#R/4.1.2
rm(list=ls())
library(ggplot2)
library(ggrepel)
...
###function to make cluster heatmap

make_gene_clusterheatmap <- function(data, genes, annotaion.col =NULL, annotation.colors, output,
                                     n.cut.row =NA, n.cut.col =NA, show.col.names = F){
  #genes = c(pc1.high, pc1.low)
  #annotaion.col = list(location = loc.col, sample = sample.colors,
  #                     PC1 = red_blue_20, PC2 = red_blue_20)
  
  gene.cluster = hclust(as.dist(1-cor(t(data[genes,]), method = "pearson")), method = "average")
  #gene.cluster = hclust(dist(data[genes,], method = "euclidean"), method = "ward.D2")
  roi.cluster = hclust(as.dist(1-cor(data[genes,], method = "pearson")), method = "average")
  #roi.cluster = hclust(dist(t(data[genes,]), method = "euclidean"), method = "ward.D2")
  png(output, width = 8, height = 8, units = "in", res = 300)
  
  print(pheatmap(data[genes,], 
                 annotation_col = annotaion.col, 
                 show_colnames = show.col.names, 
                 cluster_rows = gene.cluster, cluster_cols = roi.cluster,
                 scale = "row", 
                 color = red_blue_20, border_color = NA,
                 cutree_rows = n.cut.row, cutree_cols = n.cut.col,
                 fontsize =8, fontsize_col = n.cut.col,
...
################################################################################
load("01.CTA.allGlioma.seuratObj.rda")
#load("01.CTA.allGlioma.Q3norm.rda")
load("01.CTA.colors.shape.rda")
...
#########################################################################################################
grep("Neg", rownames(seurat), value = T)
grep("Neg", VariableFeatures(seurat), value = T)
...
#location.col <- location.col[names(location.col) %in% seurat@meta.data$location]
#location.col
...
sample.pG.colors <- sample.colors[33];sample.pG.colors
sample.colors <- c(sample.A.colors, sample.O.colors, sample.G.colors, sample.pG.colors)

seurat
tmp <- seurat[-which(rownames(seurat) == "Negative Probe"),]
tmp <- FindVariableFeatures(tmp, selection.method = "vst", nfeatures = 500)
x <- HVFInfo(tmp)
head(x)
plot(density(x$variance.standardized))
q <- quantile(x$variance.standardized, 0.80)
...
res <- cbind(seurat@meta.data, seurat@reductions$umap@cell.embeddings, seurat@reductions$pca@cell.embeddings[,1:5])
...
p2
p3 <- DimPlot(seurat, reduction = "pca", group.by = "batch",pt.size = 1, cols = batch.col)+ my.thm + labs(title = "")
p4 <- DimPlot(seurat, reduction = "pca", group.by = "grade",pt.size = 1, cols = grade.col)+ my.thm + labs(title = "")
p5 <- DimPlot(seurat, reduction = "pca", group.by = "sex", cols = pair.col[c(5,2)], pt.size = 1)+ my.thm + labs(title = "")
p6 <- DimPlot(seurat, reduction = "pca", group.by = "tumor", cols = cancer.col, pt.size = 1)+ my.thm + labs(title = "")
dev.off()


pdf("03.PCA.pdf", width = 20, height = 14, useDingbats = F)
cowplot::plot_grid(p1,p2, p3, p4, p5, p6, nrow = 2)
dev.off()

pdf("03.PCA-2.pdf", width = 16, height = 14, useDingbats = F)
DimPlot(seurat, reduction = "pca", group.by = "tumor", cols = cancer.col, pt.size = 1)+ my.thm + labs(title = "") +facet_wrap(~tumor)
ggplot(res, aes(x = PC_1, y = PC_2, color = grade))+ geom_point()+scale_color_manual(values = grade.col)+
  theme_bw()+ theme(aspect.ratio = 1)+facet_wrap(~tumor)
dev.off()
...
p1 <- DimPlot(seurat, reduction = "umap", group.by = "sample", cols = sample.colors, 
              pt.size = 1,shape.by = "location")+ my.thm + labs(x="", y = "", title = "") + scale_shape_manual(values = loc.shape)
p1
...
p3 <- DimPlot(seurat, reduction = "umap", group.by = "batch", cols = batch.col, pt.size = 1, shape.by = "location")+ my.thm + labs(title = "")+scale_shape_manual(values = loc.shape)
p4 <- DimPlot(seurat, reduction = "umap", group.by = "grade", cols = grade.col, pt.size = 1, shape.by = "location")+ my.thm + labs(x="", y = "", title = "")+scale_shape_manual(values = loc.shape)
p5 <- DimPlot(seurat, reduction = "umap", group.by = "sex", cols = pair.col[c(5,2)], pt.size = 1, shape.by = "location")+ my.thm + labs(x="", y = "", title = "")+scale_shape_manual(values = loc.shape)
p6 <- DimPlot(seurat, reduction = "umap", group.by = "tumor", cols = cancer.col, pt.size = 1, shape.by = "location")+ my.thm + labs(title = "")+scale_shape_manual(values = loc.shape)

pdf("03.umap.pdf", width = 20, height = 15, useDingbats = F)
cowplot::plot_grid(p1,p2,p3, p4, p5, p6, nrow = 2)
dev.off()

pdf("03.umap-2.pdf", width = 16, height = 14, useDingbats = F)
DimPlot(seurat, reduction = "pca", group.by = "tumor", cols = cancer.col, pt.size = 1)+ my.thm + labs(title = "") +facet_wrap(~tumor)
ggplot(res, aes(x = PC_1, y = PC_2, color = grade))+ geom_point()+scale_color_manual(values = grade.col)+
  theme_bw()+ theme(aspect.ratio = 1)+facet_wrap(~tumor)
dev.off()
...
###Plot TSNE
p1 <- DimPlot(seurat, reduction = "tsne", group.by = "sample", cols = sample.colors, pt.size = 1,shape.by = "location")+ my.thm + labs(x="", y = "", title = "")+scale_shape_manual(values = loc.shape)
p1
...
p3 <- DimPlot(seurat, reduction = "tsne", group.by = "batch", cols = batch.col, pt.size = 1, shape.by = "location")+ my.thm + labs(title = "")+scale_shape_manual(values = loc.shape)
p4 <- DimPlot(seurat, reduction = "tsne", group.by = "grade", cols = grade.col, pt.size = 1, shape.by = "location")+ my.thm + labs(x="", y = "", title = "")+scale_shape_manual(values = loc.shape)
p5 <- DimPlot(seurat, reduction = "tsne", group.by = "sex", cols = pair.col[c(5,2)], pt.size = 1, shape.by = "location")+ my.thm + labs(x="", y = "", title = "")+scale_shape_manual(values = loc.shape)
p6 <- DimPlot(seurat, reduction = "tsne", group.by = "tumor", cols = cancer.col, pt.size = 1, shape.by = "location")+ my.thm + labs(title = "")+scale_shape_manual(values = loc.shape)
pdf("03.tsne.pdf", width = 20, height = 15, useDingbats = F)
...
gene.km[1:4]
write.csv(gene.km, file = "03.variable.genes.cluster.csv", quote = F)
...
png("03.variable.genes.clusterHeatmap.png", width = 15, height = 15, units = "in", res = 100)
draw(h)
dev.off()
...
png("03.share.variable.genes.clusterHeatmap.png", width = 15, height = 15, units = "in", res = 100)
draw(h)
dev.off()
...
xx <- lapply(unique(seurat@meta.data$tumor)[4], function(Tumor){
  message(Tumor)
  #Tumor <- "GBMped"
  #dev.off()
  if(Tumor == "IDH_A"){
    ...
    sample.col <- sample.pG.colors
  }
  
  
  sub.seurat <- subset(seurat, tumor == Tumor)
  
  unique(sub.seurat$sample)
  ...
  #xx.freq
  #plot(density(xx.freq)) 
  q <- quantile(xx.freq, 0.25)
  #sum(xx.freq > q) #25% quantile
  #names(xx.freq[xx.freq > q])
  
  x <- names(xx.freq[xx.freq >= q])
  write.table(x, file = paste0("03.", Tumor, ".share.variableGenes.csv"), quote = F, col.names = F, row.names = F)
  VariableFeatures(sub.seurat) <- x
  m <- data.matrix(sub.seurat@assays$RNA@data[x,])
  m <-t(scale(t(m)))
  ...
  h <- Heatmap(m,  name = "RNA",
          ...
          show_row_names = F, show_column_names = F)
  png( paste0("03.", Tumor, ".share.variableGenes.clusterHeatmap.png"), width = 8, height = 8, units = "in", res = 100)
  draw(h)
  dev.off()
  x
...
write.csv(variable.gene.overlap, file = "04.variableGnesUpset.csv")
pdf(file = "04.variableGnesUpset.pdf", width = 15, height = 8, useDingbats = F)
...
##GBM
#sample.colors <- paletteer::paletteer_d("ggsci::category10_d3")
#sample.colors <- as.character(sample.colors)
#names(sample.colors) <- unique(seurat@meta.data$sample)
#sample.colors
...
sub.seurat <- FindVariableFeatures(sub.seurat, selection.method = "vst", nfeatures = 1000) #nrow(seurat))
VariableFeaturePlot(sub.seurat)
...
sub.seurat <- RunUMAP(sub.seurat, dims = 1:4, verbose = F, n.neighbors = 20L)
###Plot UMAP
p1 <- DimPlot(sub.seurat, reduction = "umap", group.by = "sample", cols = sample.A.colors, pt.size = 1,shape.by = "location")+ my.thm + labs(x="", y = "", title = "")+loc.shape
p1
...
#pdf("all.glioma/02.AIDH.umap.pdf", width = 20, height = 15, useDingbats = F)
pdf("all.glioma/02.AIDH.umap.pdf", width = 15, height = 10, useDingbats = F)
...
sub.seurat <- RunUMAP(sub.seurat, dims = 1:4, verbose = F, n.neighbors = 20L)
###Plot UMAP
p1 <- DimPlot(sub.seurat, reduction = "umap", group.by = "sample", cols = sample.O.colors, pt.size = 1,shape.by = "location")+ my.thm + labs(x="", y = "", title = "")+loc.shape
p1
p2 <- DimPlot(sub.seurat, reduction = "umap", group.by = "location", cols = location.col, pt.size = 1)+ my.thm + labs(x="", y = "", title = "")
p2
#p3 <- DimPlot(seurat, reduction = "umap", group.by = "Tumor", cols = cancer.col, pt.size = .5)+ my.thm + labs(title = "")
...
#pdf("all.glioma/02.AIDH.umap.pdf", width = 20, height = 15, useDingbats = F)
pdf("all.glioma/02.OIDH.umap.pdf", width = 15, height = 10, useDingbats = F)
#pdf("GBM/01.GBM.umap.pdf", width = 15, height = 10, useDingbats = F)
cowplot::plot_grid(p1,p2,p3, p4, p5, nrow = 2)
dev.off()
...
pdf("all.glioma/02.GBM.umap.pdf", width = 15, height = 10, useDingbats = F)
cowplot::plot_grid(p1,p2,p3, p4, p5, nrow = 2)
dev.off()
